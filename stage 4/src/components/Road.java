...
public class Road implements RouteParts{
	...
	public Road(Junction start, Junction end,VehicleType[] vehicleTypes) {
		startJunction=start;
		endJunction=end;
		waitingVehicles=new ArrayList<Vehicle>();
		greenLight=false;
		...
		int numOfTypes=getRandomInt(3,vehicleTypes.length);

		this.vehicleTypes=new VehicleType[numOfTypes];
		ArrayList <Integer> arr=getRandomIntArray(0,5,numOfTypes);


		for (int i=0;i<numOfTypes;i++) {
			this.vehicleTypes[i]=vehicleTypes[arr.get(i)];
		}
		this.getStartJunction().getExitingRoads().add(this);
		this.getEndJunction().getEnteringRoads().add(this);
