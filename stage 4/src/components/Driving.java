...
public class Driving implements Utilities, Timer, Runnable{
	...
	private static ArrayList<Vehicle> vehicless;
	...
	public Driving(int junctionsNum, int numOfVehicles,String x) {
		vehicles=new ArrayList<Vehicle>();
		allTimedElements=new ArrayList<Timer>();
		drivingTime=0;
		int roads = new Random().nextInt(junctionsNum*3);
		map=new Map(junctionsNum,roads,x);
		System.out.println("\n================= CREATING VEHICLES =================");
		int numberroad;
		while(vehicles.size()<numOfVehicles) {
			numberroad = getRandomInt(0,map.getRoads().size());
			while(numberroad == 0)
				numberroad = getRandomInt(0,map.getRoads().size());
			Road temp=map.getRoads().get(numberroad);//random road from the map
			if( temp.getEnabled())
				vehicles.add(new Vehicle(temp,x));
	...
	private void makevehiclesstatic(){
		vehicless = vehicles;
	}
	public static Vehicle getvehiclefromarray(int number){
		try{
			return vehicless.get(number);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public void addPrototype(Vehicle v) {
		allTimedElements.add(v);
		vehicles.add(v);
		makevehiclesstatic();
		Timer timer = allTimedElements.get(allTimedElements.size() - 1);
		Thread t = new Thread((Runnable) timer);
		t.start();
		if (threadSuspend) {
			timer.setSuspend();
