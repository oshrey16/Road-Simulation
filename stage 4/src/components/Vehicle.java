...
public class Vehicle extends Point implements Utilities, Timer, Runnable { 
	...
	private BigBrother police;
	private int speed;
	private boolean speedlimited;	//true: drive in maxspeed On road, false: Random speed 50% up to average speed
	...
	public Vehicle (Road currentLocation) {// random constructor
		...
		police = BigBrother.getInstance();
		if(new Random().nextBoolean()) {		//vehicle drive fast
			double t = vehicleType.getAverageSpeed() * 0.5;
			speed = new Random().nextInt((int) t) + vehicleType.getAverageSpeed();
		}
		else{
			speed = vehicleType.getAverageSpeed();
		}
	}

	public Vehicle (Road currentLocation,String x) {
		id = objectsCount++;
		vehicleType=null;
		int[] wheels =  {2,4,10};
		int rndwheels;
		VehicleType veht=null;
		VehicleType[] vehiclesallow;
		rndwheels = new Random().nextInt(3);		//random 0,1,2 wheels arr
		VehicleType[] v  = VehicleType.allowTypes(x);		//City or Country
		vehicleType = v[getRandomInt(0,v.length-1)];
	...
	public void calcPositionOnRoad() {
		...
		int speed;
		if(speedlimited)
			speed=Math.min(road.getMaxSpeed(), getVehicleType().getAverageSpeed());
		else
			speed = this.speed;
	...
	public synchronized void reporttopolice(){
		police.reportarrive(this);
	}

	public void randomSpeedVehicle() {
		double t = vehicleType.getAverageSpeed() * 0.5;
		speed = new Random().nextInt((int) t) + vehicleType.getAverageSpeed();

		speedlimited = new Random().nextBoolean();
	}
}
